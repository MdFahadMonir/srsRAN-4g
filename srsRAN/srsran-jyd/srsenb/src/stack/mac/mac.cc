...
void mac::get_metrics(mac_metrics_t& metrics)
{
  ...
  for (auto& u : ue_db) {
    ...
    // u.second->metrics_read(&metrics.ues.back()); // previously used
    ...
    // std::cout << "mac ri: " << ue_metrics.dl_ri << std::endl;
  }
...
int mac::ri_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t ri_value)
{
  ...
  // std::cout << "mac ri: " << ri_value << std::endl;
...
int mac::pmi_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t pmi_value)
{
  ...
  // std::cout << "mac pmi: " << pmi_value << std::endl;
...
uint16_t mac::allocate_ue(uint32_t enb_cc_idx)
{
  ...
  for (size_t i = 0; i < ue_db.size(); ++i, ++it) {
    int dl_slice_id = -1; //-1, JYD modification to force the ue attached to a slice 
...
slice_ctrl_out_e mac::slice(slice_ctrl_req_data_t const& s)
{
  return scheduler.slice(s);
}
