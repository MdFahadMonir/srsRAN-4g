...
void fill_mac_stats(mac_ind_data_t* ind)
{
  ...
  if (!m.phy.empty()) {
    printf("First PHY metric - DL samples: %d, UL samples: %d\n", 
           m.phy[0].dl.n_samples, m.phy[0].ul.n_samples);
  }
  ...
  for(size_t i = 0 ; i < sz ; ++i){
    ...
    if (i < m.phy.size()) {
      printf("PHY metrics available: UE %zu, DL MCS %.1f, UL MCS %.1f\n", i, m.phy[i].dl.mcs, m.phy[i].ul.mcs);
      dst->dl_mcs1 = (uint8_t)std::max(0, (int)std::lround(m.phy[i].dl.mcs));
      dst->ul_mcs1 = (uint8_t)std::max(0, (int)std::lround(m.phy[i].ul.mcs));

      // Get SNR from PHY layer PUSCH/PUCCH metrics
      float pusch_sinr = m.phy[i].ul.pusch_sinr;
      float pucch_sinr = m.phy[i].ul.pucch_sinr;

      // Handle PUSCH SNR
      if (!std::isnan(pusch_sinr) && !iszero(pusch_sinr)) {
        dst->pusch_snr = std_clamp<float>(pusch_sinr, -99.9f, 99.9f);
      } else {
        dst->pusch_snr = 0.0f; // Default value when SNR is not available
      }

      // Handle PUCCH SNR
      if (!std::isnan(pucch_sinr) && !iszero(pucch_sinr)) {
        dst->pucch_snr = std_clamp<float>(pucch_sinr, -99.9f, 99.9f);
      } else {
        dst->pucch_snr = 0.0f; // Default value when SNR is not available
      }
