...
bool enb::get_metrics(enb_metrics_t* m)
{
  ...
  // print enb_metrics pointer m
  // printf("Printed by both enb and xappp, enb_metrics pointer: %p\n", m);
  if (!started) {
    return false;
  }
  ...
  // print phy metrics
  //printf("Printed by both enb and xappp, phy_metrics pointer: %f\n", m->phy.rx_rssi_);
...
enb_stack_base& enb::get_eutra_stack()
{
  assert(eutra_stack != NULL);
  return *eutra_stack;  
}
