...
{
  //printf("phy::get_metrics called\n");  
  //Info("Getting PHY metrics, nof_workers=%d", nof_workers);
  std::vector<phy_metrics_t> metrics_tmp;
  for (uint32_t i = 0; i < nof_workers; i++) {
    lte_workers[i]->get_metrics(metrics_tmp);
    //Info("Worker %d returned %zu metrics", i, metrics_tmp.size());
    metrics.resize(std::max(metrics_tmp.size(), metrics.size()));
    for (uint32_t j = 0; j < metrics_tmp.size(); j++) {
      //printf("Worker %d Before accumulation - DL samples=%d, mcs=%.2f | UL samples=%d, mcs=%.2f\n",
      //       i, metrics_tmp[j].dl.n_samples, metrics_tmp[j].dl.mcs,
      //       metrics_tmp[j].ul.n_samples, metrics_tmp[j].ul.mcs);

      uint32_t prev_dl_samples = metrics[j].dl.n_samples;
      uint32_t prev_ul_samples = metrics[j].ul.n_samples;
      uint32_t prev_ul_pucch_samples = metrics[j].ul.n_samples_pucch;
      ...
      if (metrics_tmp[j].dl.n_samples > 0 && metrics[j].dl.n_samples > 0) {
          metrics[j].dl.mcs = (prev_dl_samples * metrics[j].dl.mcs + 
                              metrics_tmp[j].dl.n_samples * metrics_tmp[j].dl.mcs) / 
                             metrics[j].dl.n_samples;
      }
      
      if (metrics_tmp[j].ul.n_samples > 0 && metrics[j].ul.n_samples > 0) {
          metrics[j].ul.mcs = (prev_ul_samples * metrics[j].ul.mcs +
                              metrics_tmp[j].ul.n_samples * metrics_tmp[j].ul.mcs) /
                             metrics[j].ul.n_samples;
          metrics[j].ul.n = (prev_ul_samples * metrics[j].ul.n +
                          metrics_tmp[j].ul.n_samples * metrics_tmp[j].ul.n) /
                         metrics[j].ul.n_samples;
          metrics[j].ul.rssi = (prev_ul_samples * metrics[j].ul.rssi +
                             metrics_tmp[j].ul.n_samples * metrics_tmp[j].ul.rssi) /
                            metrics[j].ul.n_samples;
          metrics[j].ul.pusch_sinr = (prev_ul_samples * metrics[j].ul.pusch_sinr +
                                   metrics_tmp[j].ul.n_samples * metrics_tmp[j].ul.pusch_sinr) /
                                  metrics[j].ul.n_samples;
          metrics[j].ul.turbo_iters = (prev_ul_samples * metrics[j].ul.turbo_iters +
                                    metrics_tmp[j].ul.n_samples * metrics_tmp[j].ul.turbo_iters) /
                                   metrics[j].ul.n_samples;
      }
      
      if (metrics_tmp[j].ul.n_samples_pucch > 0 && metrics[j].ul.n_samples_pucch > 0) {
          metrics[j].ul.pucch_sinr = (prev_ul_pucch_samples * metrics[j].ul.pucch_sinr +
                                   metrics_tmp[j].ul.n_samples_pucch * metrics_tmp[j].ul.pucch_sinr) /
                                  metrics[j].ul.n_samples_pucch;
      }

      //printf("Worker %d After accumulation - Total DL samples=%d, mcs=%.2f | UL samples=%d, mcs=%.2f\n",
      //       i, metrics[j].dl.n_samples, metrics[j].dl.mcs,
      //       metrics[j].ul.n_samples, metrics[j].ul.mcs);
    }
  }
  for (uint32_t j = 0; j < metrics.size(); j++) {
    //printf("Before averaging - DL total_mcs=%.2f, samples=%d | UL total_mcs=%.2f, samples=%d\n",
    //       metrics[j].dl.mcs, metrics[j].dl.n_samples,
    //       metrics[j].ul.mcs, metrics[j].ul.n_samples);

    // No need to average again since we already computed weighted averages during accumulation

    //printf("Final metrics - DL mcs=%.2f (samples=%d) | UL mcs=%.2f (samples=%d)\n",
    //       metrics[j].dl.mcs, metrics[j].dl.n_samples,
    //       metrics[j].ul.mcs, metrics[j].ul.n_samples);
  }
