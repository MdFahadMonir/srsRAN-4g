    ...
    if (SRSRAN_RNTI_ISUSER(rnti) and phy->ue_db.is_pcell(rnti, cc_idx)) {
      ...
      if (ret > SRSRAN_SUCCESS) {
        ...
        if (pucch_res.detected and pucch_res.ta_valid) {
          ...
          // Debug: Print raw PUCCH SNR from channel estimator
          //printf("RAW PUCCH SNR: rnti=0x%x, pucch_res.snr_db=%.2f dB\n", rnti, pucch_res.snr_db);
        }
...
uint32_t cc_worker::get_metrics(std::vector<phy_metrics_t>& metrics)
{
  ...
  //printf("CC Worker: Collecting metrics from %zu UEs\n", ue_db.size());
  for (auto& ue : ue_db) {
...
{
  if (metrics_) {
    // Save the metrics to the output parameter
    *metrics_ = metrics;
  }
  
  // Do NOT reset metrics anymore since we want to maintain the running averages
  // The metrics will be updated properly in metrics_dl() and metrics_ul()
}

void cc_worker::ue::metrics_dl(uint32_t mcs)
{
  if (metrics.dl.n_samples == 0) {
    metrics.dl.mcs = mcs;
  } else {
    float alpha = 0.1; // Weight for new samples in moving average
    metrics.dl.mcs = (1 - alpha) * metrics.dl.mcs + alpha * mcs;
  }
...
{
  if (metrics.ul.n_samples == 0) {
    metrics.ul.mcs = mcs;
    metrics.ul.pusch_sinr = sinr;
    metrics.ul.rssi = rssi;
    metrics.ul.turbo_iters = turbo_iters;
  } else {
    float alpha = 0.1; // Weight for new samples in moving average
    metrics.ul.mcs = (1 - alpha) * metrics.ul.mcs + alpha * mcs;
    metrics.ul.pusch_sinr = (1 - alpha) * metrics.ul.pusch_sinr + alpha * sinr;
    metrics.ul.rssi = (1 - alpha) * metrics.ul.rssi + alpha * rssi;
    metrics.ul.turbo_iters = (1 - alpha) * metrics.ul.turbo_iters + alpha * turbo_iters;
  }
...
{
  if (metrics.ul.n_samples_pucch == 0) {
    metrics.ul.pucch_sinr = sinr;
  } else {
    float alpha = 0.1; // Weight for new samples in moving average
    metrics.ul.pucch_sinr = (1 - alpha) * metrics.ul.pucch_sinr + alpha * sinr;
  }
