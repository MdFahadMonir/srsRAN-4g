...
void phy::get_metrics(std::vector<phy_metrics_t>& metrics)
{
  ...
  for (uint32_t i = 0; i < nof_workers; i++) {
    ...
    for (uint32_t j = 0; j < metrics_tmp.size(); j++) {
      ...
      // print ul metrics mcs
      // print metric temp
      printf("Printed by enb only, ul_metrics mcs from worker %d: %f\n", i, metrics_tmp[j].ul.mcs);
    }
  }
  for (uint32_t j = 0; j < metrics.size(); j++) {
    metrics[j].dl.mcs /= metrics[j].dl.n_samples;
        //metrics[j].dl.mcs = 100;
    ...
    printf("Printed by both enb and xappp, dl_metrics mcs: %f\n", metrics[j].dl.mcs);
    printf("Printed by both enb and xappp, ul_metrics mcs: %f\n", metrics[j].ul.mcs);
  }
